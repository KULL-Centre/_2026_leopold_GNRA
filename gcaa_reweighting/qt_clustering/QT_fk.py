...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description=desc, add_help=True, epilog="As simple as that ;)"
    )
    ...
    parser.add_argument(
        ...
        dest="matrix_loc",
        action="store",
        help="path to precomputed matrix",
        type=str,
        required=False,
        default=None,
    )
...
def load_trajectory(args):
    ...
    if args.selection != "all":
        ...
        trajectory = trajectory.atom_slice(sel_indx)[
            args.first : args.last : args.stride
        ]
    else:
...
if inputs.trajectory != None:
    ...
    matrix = load_bz2_pkl(inputs.matrix_loc)
    N = matrix.shape[0]
    print(N)
...
with open("QT_Visualization.log", "wt") as clq:
    for numcluster in np.unique(clusters_arr):
        clq.write("{}:\n".format(numcluster))
        members = " ".join([str(x) for x in np.where(clusters_arr == numcluster)[0]])
