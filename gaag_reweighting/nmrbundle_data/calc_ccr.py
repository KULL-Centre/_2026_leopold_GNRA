...
As = np.array(
    [
        [0.733, -0.5281, -0.4287],
        [0.0443, -0.5919, 0.8048],
        [-0.6787, -0.6090, -0.4105],
    ]
)
A1 = np.array(
    [
        [0.6345, -0.4976, -0.5915],
        [0.1577, -0.6658, 0.7293],
        [-0.7567, -0.5560, -0.4105],
    ]
)
...
#B0 = 16.44  #in Tesla field strenght, 700Mhz
B0 = 22.30  #in Tesla field strenght, 950Mhz
tc = 2.27e-9  # tau c, ns
...
ccr_p_const = (
    (-2.0 * gyro["H"] * gyro["C"] * gyro["P"] * mu0 * hbar * tc * B0)
    / (15 * r_CH_cube)
    * 1e-6
)
...
# Reads cmd args and returns trajectories
def read_args():
    if len(sys.argv) == 2:
        return md.load(sys.argv[1])
    if len(sys.argv) == 3:
        return md.load(sys.argv[1], top=sys.argv[2])
    raise Exception("Invalid arguments")
    # t = md.load("1zih.pdb")
    # t = md.load("nmrbundle_data/Bundle_1_3_ccr.pdb")


# What is this?
def calc_shit(v_c, sigma_22_vec, sigma_33_vec):
    ...
    legendre_1 = (sigma_22 - sigma_11) * (3.0 * cos_theta_1 * cos_theta_1 - 1.0)
    legendre_2 = (sigma_33 - sigma_11) * (3.0 * cos_theta_2 * cos_theta_2 - 1.0)

    gamma = ccr_p_const * (legendre_1 + legendre_2)
    return gamma


def calc(traj, renaming={}):
    ...
    for at in traj.topology.atoms:
        if at.name in req_atoms:
            ...
            # print(f"atom name {str(at)}")
            at_string = str(at)
            at_list = at_string.split("-")
            if renaming and at_list[1] in renaming:
                print(f"renaming {str(at)} to {at_list[0] +'-'+ renaming[at_list[1]]}")
                req_atoms_name.append(at_list[0] + "-" + renaming[at_list[1]])
            else:
                req_atoms_name.append(at_string)
    # print(f"idx: \n{req_atoms_idx}")
    ##print(f"names: \n{req_atoms_name}")
    ...
    for res in traj.topology.residues:
        ...
        try:
            c5h1 = req_atoms_name.index("%s-H5'1" % res)
            c5h2 = req_atoms_name.index("%s-H5'2" % res)
        ...
        if res.name == "URI" or res.name == "C" or res.name == "U":
    ...
    for res in traj.topology.residues:
        g1 = "%s:C1-C2" % res
        ccr_names[g1] = [
            bonds_names.index("%s:C1p" % res),
            bonds_names.index("%s:C2p" % res),
        ]
        ccr_const[g1] = (
            gyro["H"] * gyro["H"] * gyro["C"] * gyro["C"] * mu0 * mu0 * hbar * hbar * tc
        ) / (5.0 * r_CH_cube * r_CH_cube)

        g2 = "%s:C3-C4" % res
        ccr_names[g2] = [
            bonds_names.index("%s:C3p" % res),
            bonds_names.index("%s:C4p" % res),
        ]
        ccr_const[g2] = (
            gyro["H"] * gyro["H"] * gyro["C"] * gyro["C"] * mu0 * mu0 * hbar * hbar * tc
        ) / (5.0 * r_CH_cube * r_CH_cube)
        ...
        ccr_names[g3] = [
            bonds_names.index("%s:C1p" % res),
            bonds_names.index("%s:CC" % res),
        ]
        ccr_const[g3] = (
            gyro["H"] * gyro["H"] * gyro["C"] * gyro["C"] * mu0 * mu0 * hbar * hbar * tc
        ) / (5.0 * r_CH_cube * r_CH_chi_cube)
        columns.extend([g1, g2, g3])

    for res in traj.topology.residues:

        if res.index != 0:
            r_minus = traj.topology.residue(res.index - 1)
    ...
    coords = traj.xyz[:, req_atoms_idx]
    bond_vecs = coords[:, bonds[:, 1]] - coords[:, bonds[:, 0]]

    kk = bonds_names.index("G2:C5p")

    # print(coords[0,bonds[kk,1]])
    # print(coords[0,bonds[kk,0]])
    # tt = coords[0,bonds[kk,1]]-coords[0,bonds[kk,0]]
    # tt /= np.linalg.norm(tt)
    # print(tt,np.linalg.norm(tt))
    # normalize
    ...
    for frame in range(coords.shape[0]):
        ...
        for el in ccr_names:
            ...
            legendre = 3.0 * cos_theta * cos_theta - 1.0
        ...
        for res in traj.topology.residues:
            ...
            if res.index != 0:
                ...
                data_tmp.append(calc_shit(v_c, sigma_22_vec, sigma_33_vec))
