...
As = np.array(
    [[0.733, -0.5281, -0.4287], [0.0443, -0.5919, 0.8048], [-0.6787, -0.6090, -0.4105]]
)
A1 = np.array(
    [[0.6345, -0.4976, -0.5915], [0.1577, -0.6658, 0.7293], [-0.7567, -0.5560, -0.4105]]
)
...
df.to_csv(
    sys.argv[1].split(".")[0] + "_calc.dat", header=True, sep="\t", float_format="%8.4e"
)  #    md.load("1zih.pdb")     sys.argv[1] #index_label="frame"
