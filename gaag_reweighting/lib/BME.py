...
class Reweight:
    ...
    def read_file(
        ...
        # read file
        log = ""
        label, exp, calc, log, averaging = bt.parse(
            exp_file, calc_file, averaging=averaging
        )
        self.log.write(log)
    ...
    def predict_array(
        self, label, exp, calc, outfile=None, averaging="linear", fit="no"
    ):
        stats, log = bt.calc_stats(
            label,
            exp,
    ...
    def fit(self, theta, lambdas_init=True, method="BME"):
        ...
        if self.standardized == False:
            log, v1, v2 = bt.standardize(
                self.experiment, self.calculated, self.w0, normalize="zscore"
            )
            self.standardized = True

        assert method in known_methods, "method %s not in known methods:" % (
            method,
            known_methods,
        )
        ...
        def maxent(lambdas):
            # weights
            arg = (
                -np.sum(lambdas[np.newaxis, :] * self.calculated, axis=1)
                - tmax
                + np.log(self.w0)
            )
        ...
        def maxent_hess(lambdas):
            ...
            assert np.isfinite(
                zz
            ), "# Error. sum of weights is infinite. Use higher theta"
            ww /= zz

            q_w = np.dot(ww, self.calculated)
            hess = (
                np.einsum("k, ki, kj->ij", ww, self.calculated, self.calculated)
                - np.outer(q_w, q_w)
                + np.diag(theta_sigma2)
            )
        ...
        def func_ber_gauss(w):
            ...
            jac = np.sum(diff * self.calculated, axis=1) + theta * (1.0 + log_div)
        ...
        def func_chi2_L2(w):
            ...
            ii = np.where(
                ((diff < 0) & (self.experiment[:, 2] < 0))
                | ((diff > 0) & (self.experiment[:, 2] > 0))
            )[0]
            ff = [
                1 if (self.experiment[j, 2] == 0 or j in ii) else 0
    ...
    def theta_scan(
        self, thetas=[], train_fraction_data=0.75, nfold=5, train_fraction_samples=0.8
    ):
        ...
        if len(thetas) == 0:
            thetas = np.geomspace(0.1, 10000, 10)
        print(
            "Performing %d-fold cross validation for %d theta values"
            % (nfold, len(thetas))
        )
        ...
        for i in range(nfold):
            ...
            test_idx_samples = shuffle_samples[
                :train_samples
            ]  # test samples are the same as train!
            ...
            r1 = Reweight(
                "crossval_%s_%d" % (self.name, i),
                w0=np.copy(self.w0[train_idx_samples]),
            )
            r1.load_array(
                labels_train,
                np.copy(exp_train),
                np.copy(calc_train[train_idx_samples, :]),
            )
            l_init = True
            ...
            for j, t in enumerate(thetas):
                ...
                train_stats = r1.predict_array(
                    labels_train,
                    exp_train,
                    calc_train[train_idx_samples, :],
                    outfile="%s/%s_train" % (tmp_dir, fr),
                )
                test_stats = r1.predict_array(
                    labels_test,
                    exp_test,
                    calc_test[test_idx_samples, :],
    ...
    def ibme(self, theta, ftol=0.01, iterations=50, lr_weights=True, offset=True):
        ...
        for it in range(iterations):
            ...
            line = (
                "Iteration:%3d scale: %7.4f offset: %7.4f chi2: %7.4f diff: %7.4e\n"
            ...
            if diff < ftol:
                line = (
                    "Iterative procedure converged below tolerance %.2e after %d iterations\n"
                    % (diff, it)
                )
                print(line, end="")
                log += line
    ...
    def iterative_theta_scan(
        ...
        lr_weights=True,
        offset=True,
        train_fraction_data=0.75,
        nfold=5,
        train_fraction_samples=0.8,
    ):
        ...
        for i in range(nfold):
            ...
            for j, t in enumerate(thetas):
                c1, c2, phi, calc_0, calc_1 = r1.ibme(
                    t,
                    ftol=ftol,
                ...
                chi_test_0 = bt.calc_chi(
                    exp[test_idx_data], calc_test_0[test_idx_samples, :], r1.w0
                )
                chi_test_1 = bt.calc_chi(
                    exp[test_idx_data], calc_test_1[test_idx_samples, :], r1.w_opt
                )
                # print("####",i,j)
                # outfiles.append(fr)
